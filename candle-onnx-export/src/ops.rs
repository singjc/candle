...
/// Applies a Candle-style linear layer to the last dimension of a rank-3 tensor using 2D `Gemm`.
///
/// The emitted graph is:
///
/// `input [batch, seq, in] -> Flatten(axis=2) -> Gemm -> Reshape [batch, seq, out]`
///
/// This is useful for CPU runtimes where a large 2D `Gemm` can be faster than a batched
/// rank-3 `MatMul` followed by a separate bias `Add`. The supplied weight must use Candle's
/// `Linear` convention, i.e. `[out_features, in_features]`, and the optional bias must be
/// `[out_features]`.
pub fn linear_last_dim_gemm(
    graph: &mut OnnxGraph,
    input: Value,
    weight: &str,
    bias: Option<&str>,
    ...
    if output_features <= 0 {
        return Err(Error::InvalidGraph(format!(
            "linear_last_dim_gemm output_features must be positive, got {output_features}"
        )));
    }
    ...
    let output_shape = linear_last_dim_output_shape(
        graph,
        input.clone(),
        output_features,
        &format!("{output_name}_shape"),
    )?;
    let flat = flatten(graph, input.clone(), 2, &format!("{output_name}_flat"))?;
    let flat_output = linear(
        graph,
        flat,
        weight,
        bias,
        &format!("{output_name}_gemm"),
    )?;
    let mut output = reshape(graph, flat_output, output_shape, output_name)?;

    if input.shape.dims.len() >= 2 {
        output.shape = Shape::from_dims(vec![
            input.shape.dims[0].clone(),
            input.shape.dims[1].clone(),
            Dim::Fixed(output_features),
        ]);
    }
...
fn linear_last_dim_output_shape(
    graph: &mut OnnxGraph,
    input: Value,
    output_features: i64,
    output_name: &str,
) -> Result<Value> {
    let input_shape = shape(graph, input, &format!("{output_name}_input"))?;
    let leading_shape = slice_i64(
        graph,
        input_shape,
        &[0],
        &[2],
        Some(&[0_i64][..]),
        None,
        &format!("{output_name}_leading"),
    )?;
    let output_dim_name = graph.unique_name(&format!("{output_name}_out_dim"));
    let output_dim = graph.add_initializer_value(TensorData::vec_i64(
        output_dim_name,
        &[output_features],
    )?);
    concat(
        graph,
        &[leading_shape, output_dim],
        0,
        &format!("{output_name}_target"),
    )
}
